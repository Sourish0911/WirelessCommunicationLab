...
for i = 1:length(snr_db)
    ...
    for j = 1:N/2
        ...
        w1 = h_releigh_rx1(j)*conj(h_releigh_rx1(j)) / (h_releigh_rx1(j)*conj(h_releigh_rx1(j)) + h_releigh_rx2(j)*conj(h_releigh_rx2(j)));
        w2 = h_releigh_rx2(j)*conj(h_releigh_rx2(j)) / (h_releigh_rx1(j)*conj(h_releigh_rx1(j)) + h_releigh_rx2(j)*conj(h_releigh_rx2(j)));
        qpsk_symbols_combined(j) = w1*qpsk_symbols_rx1(j)+w2*qpsk_symbols_rx2(j);
    end
...
for i = 1:length(snr_db)
    ...
    for j = 1:N/2
        ...
        % Perform maximum ratio combining
        % Perform maximum ratio combining  
        w1 = h_naka_rx1(j)*conj(h_naka_rx1(j)) / (h_naka_rx1(j)*conj(h_naka_rx1(j)) + h_naka_rx2(j)*conj(h_naka_rx2(j)));
        w2 = h_naka_rx2(j)*conj(h_naka_rx2(j)) / (h_naka_rx1(j)*conj(h_naka_rx1(j)) + h_naka_rx2(j)*conj(h_naka_rx2(j)));
        qpsk_symbols_combined(j) = w1*qpsk_symbols_rx1(j)+w2*qpsk_symbols_rx2(j);
    end
    qpsk_demod_r = qpsk_demod(qpsk_symbols_combined,N/2);
